...
const { getReporterColorMap } = require('internal/test_runner/colors');
...
const reporterColorMap = {
  '__proto__': null,
  get 'test:fail'() {
    return getReporterColorMap().get('test:fail') || colors.red;
  },
  get 'test:pass'() {
    return getReporterColorMap().get('test:pass') || colors.green;
  },
  get 'test:diagnostic'() {
    return getReporterColorMap().get('test:diagnostic') || colors.blue;
  },
  get 'info'() {
    return getReporterColorMap().get('info') || colors.blue;
  },
  get 'warn'() {
    return getReporterColorMap().get('warn') || colors.yellow;
  },
  get 'error'() {
    return getReporterColorMap().get('error') || colors.red;
  },
};
