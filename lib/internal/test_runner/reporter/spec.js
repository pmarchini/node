...
const { kBailedOut, kSubtestsFailed } = require('internal/test_runner/test');
    ...
    switch (type) {
      case 'test:fail':
        if (data.details?.error?.failureType !== kSubtestsFailed &&
            data.details?.error?.failureType !== kBailedOut) {
      ...
      case 'test:bailout':
        return `${reporterColorMap['test:fail']}${reporterUnicodeSymbolMap['test:fail']}Bail out!` +
               `${data.reason ? ` ${data.reason}` : ''}${colors.white}\n`;
      case 'test:coverage':
