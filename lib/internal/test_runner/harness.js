...
function captureConsoleOutput(root) {
  ...
  console[kBindStreamsLazy]({
    __proto__: null,
    stdout: new TestOutputStream(root.reporter, 'stdout'),
    stderr: new TestOutputStream(root.reporter, 'stderr'),
  });
...
function setupProcessState(root, globalOptions) {
  ...
  // Only processes that execute test code capture the console: a child spawned
  // by the runner, or the runner itself when isolation is 'none'.
  const executesTests = process.env.NODE_TEST_CONTEXT !== undefined ||
