...
const { kBindStreamsLazy } = require('internal/console/constructor');
...
// Receives what the global console writes while tests run in this process and
// reports it as a test:stdout/test:stderr event of the test that produced it,
// so reporters see it in order with that test's other events.
class TestOutputStream extends Writable {
  #reporter;
  #type;
  #colorize;

  constructor(reporter, type, colorize) {
  ...
  // The console colorizes only when its stream is a color-capable TTY. The
  // captured output ends up on the reporter's destination, so answer with the
  // decision the runner already makes when it sets FORCE_COLOR for children.
  get isTTY() {
  ...
  _write(chunk, encoding, callback) {
    ...
    // Only output produced inside a running test is reported through it.
    // Everything else, including what reporters and consumers of the events
    // stream log themselves, keeps going to the real stream.
    if (!(test instanceof Test)) {
      process[this.#type].write(chunk, encoding, callback);
      return;
    }

    this.#reporter[this.#type](test.loc?.file, chunk);
    callback();
  }
}
...
function captureConsoleOutput(root) {
  // A run() started from inside a test leaves the console bound to the outer
  // tree: whatever that test logs while consuming the inner events is its own
  // output, and routing it into the inner stream would feed it back to itself.
  if (isConsoleCaptured) {
    return () => {};
  }

  const { reporter, harness: { shouldColorizeTestFiles: colorize } } = root;
  const console = require('internal/console/global');
  console[kBindStreamsLazy]({
    __proto__: null,
    stdout: new TestOutputStream(reporter, 'stdout', colorize),
    stderr: new TestOutputStream(reporter, 'stderr', colorize),
  });
...
function setupProcessState(root, globalOptions) {
  ...
  root.harness.restoreConsole = captureConsoleOutput(root);
}
