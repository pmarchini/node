...
const { white, shouldColorize } = require('internal/util/colors');
...
const { getCoverageColorMap } = require('internal/test_runner/colors');

function getCoverageColors() {
...
function getCoverageReport(pad, summary, symbol, color, table) {
  ...
  function getCell(string, width, pad, truncate, coverage) {
    ...
    if (color && coverage !== undefined) {
      const coverageColors = getCoverageColors();
