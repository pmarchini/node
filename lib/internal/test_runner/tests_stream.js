...
const { reporterScope } = require('internal/test_runner/utils');
...
class TestsStream extends Readable {
  ...
  [kEmitMessage](type, data) {
    // Listeners act as reporters: running them in the reporter scope keeps
    // their own console output out of the stream they are consuming.
    reporterScope.runInAsyncScope(this.emit, this, type, data);
