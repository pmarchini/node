...
function bailError() {
  return new ERR_TEST_FAILURE(
    'test was cancelled because the test run bailed out',
    kBailedOut,
  );
}
...
class Test extends AsyncResource {
  ...
  async processPendingSubtests() {
    if (this.root.harness.bailedOut) {
      // Report the bailout before cancelling the queued subtests so that the
      // 'Bail out!' line directly follows the failure that caused it.
      this.#reportBailout();
      this.#cancelPendingSubtests(bailError());
      return;
    }
  ...
  #cancelPendingSubtests(error) {
    while (this.pendingSubtests.length > 0) {
      const deferred = ArrayPrototypeShift(this.pendingSubtests);
      const subtest = deferred.test;

      subtest.#cancel(error);
      subtest.postRun(error);
      deferred.resolve();
    }
  }
  ...
  bail(failedTestName) {
    ...
    if (harness.bailedOut) {
      return;
    }

    harness.bailedOut = true;
    harness.bailoutReason = failedTestName;
    // Defer the cancellation pass with a microtask so that the reporting of
    // the failing test completes before the remaining tests are cancelled.
    queueMicrotask(() => {
      root.#reportBailout();
      root.#bailOutstandingSubtests(bailError());
    });
  }
  ...
  #reportBailout() {
    ...
    if (harness.bailoutReported) {
      return;
    }

    harness.bailoutReported = true;
    root.reporter.bailout(root.nesting, root.loc, harness.bailoutReason);
  }

  #bailOutstandingSubtests(error) {
    // Cancel queued subtests that have not started yet, and abort the ones
    // that are in flight. In flight subtests observe the abort and invoke
    // their own postRun().
    this.#cancelPendingSubtests(error);

    for (let i = 0; i < this.subtests.length; i++) {
      const subtest = this.subtests[i];

      if (subtest.startTime !== null && !subtest.finished) {
        subtest.#bailOutstandingSubtests(error);
        subtest.#cancel(error);
      }
    }
  }
  ...
  start() {
    ...
    if (this.root.harness.bailedOut ||
        this.root.harness.buildPromise ||
  ...
  postRun(pendingSubtestsError) {
    ...
    if (this.config.bail && this.parent !== null &&
        !this.root.harness.bailedOut &&
        !this.passed && !this.skipped && !this.isTodo &&
        this.error?.failureType !== kAborted &&
        this.error?.failureType !== kCancelledByParent) {
      this.root.bail(this.name);
    }
