...
class Test extends AsyncResource {
  ...
  #abortHandler = () => {
    const rootSignalReason = this.root?.abortController?.signal?.reason;
    const reason = this.outerSignal?.reason || rootSignalReason;
    let error;
    if (rootSignalReason === kBailed) {
      error = new AbortError('The test was aborted due to --test-bail');
      error.failureType = kBailed;
    } else if (reason instanceof Error) {
      error = reason;
      error.failureType = kAborted;
    } else {
      error = new AbortError('The test was aborted');
      error.failureType = kAborted;
    }
    this.#cancel(error);
  };
  ...
  [kShouldAbort]() {
    if (this.signal.aborted || this.outerSignal?.aborted || this.root?.abortController?.signal?.aborted) {
      this.#abortHandler();
      return true;
    }
  }
  ...
  async run() {
    ...
    try {
      ...
      if (this.parent !== null) {
        ...
        // If the bail option is enabled and this test has failed, abort the root
        // harness to prevent any further tests from running.
        if (this.config.bail && this.error) {
          this.root.abortController.abort(kBailed);
        }
      }
    }
