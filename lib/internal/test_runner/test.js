...
const { AsyncResource, executionAsyncId, createHook } = require('async_hooks');
...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    this.promiseHook = null;
    this.trackedPromises = new SafeMap();
    this.testAsyncId = null;
  ...
  #setupPromiseTracking() {
    // Store the async ID of this test's async resource
    this.testAsyncId = this.asyncId();

    const self = this;
    this.promiseHook = createHook({
      init(asyncId, type, triggerAsyncId) {
        // Only track promises created in the execution context of this test
        if (type === 'PROMISE') {
          const currentAsyncId = executionAsyncId();
          // Track if the promise was created while this test was executing
          if (currentAsyncId === self.testAsyncId || 
              self.trackedPromises.has(triggerAsyncId)) {
            // Capture stack trace to show where the promise was created
            const callSites = getCallSites();
            
            self.trackedPromises.set(asyncId, {
              __proto__: null,
              triggerAsyncId,
      ...
      before(asyncId) {
        // before() is called just before promise callbacks execute
        const promiseInfo = self.trackedPromises.get(asyncId);
        if (promiseInfo !== undefined) {
          promiseInfo.callbackExecuted = true;
          promiseInfo.callbackStartedAt = hrtime();
        }
      },
      after(asyncId) {
        // after() is called just after promise callbacks finish
        // Remove from tracking since we know it's no longer pending
        self.trackedPromises.delete(asyncId);
      },
      destroy(asyncId) {
        // Clean up any promises that weren't removed in after()
        self.trackedPromises.delete(asyncId);
      },
    });

    this.promiseHook.enable();
  }

  #cleanupPromiseTracking() {
    if (this.promiseHook !== null) {
      this.promiseHook.disable();
      this.promiseHook = null;
    }
  }
  ...
  #getPendingPromises() {
    const pending = [];
    const now = hrtime();
    for (const [asyncId, info] of this.trackedPromises.entries()) {
      // A promise is considered "pending" if its callback hasn't been executed
      // Note: before() hook is called when promise callbacks start executing
      if (!info.callbackExecuted) {
        // Calculate how long the promise has been pending (in milliseconds)
        const pendingDuration = Number(now - info.createdAt) / 1_000_000;
        
        // Check if this promise has any user code in its stack
        ...
        if (info.callSites) {
          for (let i = 0; i < info.callSites.length && frameCount < maxFrames; i++) {
            const callSite = info.callSites[i];
            const fileName = callSite.scriptName;
            
            // Skip internal async_hooks and test runner frames
            if (fileName && (
              StringPrototypeStartsWith(fileName, 'node:internal/async_hooks') ||
              StringPrototypeStartsWith(fileName, 'node:internal/test_runner/test')
            )) {
              continue;
            }
            
            // Check if this is user code (not node:internal/*)
            const isUserCode = fileName && !StringPrototypeStartsWith(fileName, 'node:internal/');
            if (isUserCode) {
              hasUserCode = true;
            }
            
            const functionName = callSite.functionName || '<anonymous>';
            const lineNumber = callSite.lineNumber;
            const columnNumber = callSite.columnNumber;
        ...
        // Only include promises that have user code in their stack trace
        if (hasUserCode) {
  ...
  async run() {
    ...
    // Set up promise tracking for this test
  ...
  postRun(pendingSubtestsError) {
    ...
    // Report pending promises if there's a pendingSubtestsError
    if (pendingSubtestsError) {
      const pendingPromises = this.#getPendingPromises();
      if (pendingPromises.length > 0) {
        const promiseCount = pendingPromises.length;
        const promiseString = `promise${promiseCount > 1 ? 's' : ''}`;
        let diagnosticMsg = `${promiseCount} unresolved ${promiseString} remaining in test scope:`;
        
        for (let i = 0; i < pendingPromises.length; i++) {
          const { asyncId, triggerAsyncId, stack, pendingDuration } = pendingPromises[i];
          diagnosticMsg += `\n\n  Promise #${i + 1} (asyncId: ${asyncId}, triggered by: ${triggerAsyncId})`;
          diagnosticMsg += `\n  Pending for: ${NumberPrototypeToFixed(pendingDuration, 2)}ms`;
