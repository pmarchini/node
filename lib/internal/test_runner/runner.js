...
class FileTest extends Test {
  ...
  #handleReportItem(item) {
    if (item.type === 'test:bailout') {
      // The parent process emits a single bailout event of its own.
      return;
    }
  ...
  #accumulateReportItem(item) {
    ...
    if (item.type === 'test:fail' && this.config.bail &&
        !this.root.harness.bailedOut &&
        item.data.todo === undefined && item.data.skip === undefined &&
        !kCanceledTests.has(item.data.details?.error?.failureType)) {
      this.root.bail(item.data.name);
    }
  }
  #drainReportBuffer() {
...
function run(options = kEmptyObject) {
  ...
  if (bail != null) {
    ...
    if (bail && watch) {
      throw new ERR_INVALID_ARG_VALUE(
        'options.bail', watch, 'is not supported with watch mode',
      );
    }
  }
  if (only != null) {
  ...
  const globalOptions = {
    ...
    bail: bail ?? parseCommandLine().bail,
    coverage,
    coverageExcludeGlobs,
    coverageIncludeGlobs,
