...
// Helper function to create a list of test files
function createTestFileList(patterns, cwd) {
  const hasUserSuppliedPattern = patterns != null;
  if (!patterns || patterns.length === 0) {
    patterns = [kDefaultPattern];
...
// Helper function to validate run options
...
// Helper function to collect test files
function collectTestFiles({ files, globPatterns, cwd, shard }) {
...
// Helper function to initialize the test tree
function initializeTestTree({ concurrency, timeout, signal, setup }) {
  ...
  return createTestTree(rootTestOptions, globalOptions);
}

// Helper function to set up watch mode
function setupWatchMode({ testFiles, opts }) {
  const runningProcesses = new SafeMap();
  const runningSubtests = new SafeMap();
  const watcherMode = opts.hasFiles ? 'filter' : 'all';
  const watcher = new FilesWatcher({ __proto__: null, debounce: 200, mode: watcherMode, signal: opts.signal });
...
// Helper function to run tests in process isolation
function runTestsInProcessIsolation({
  testFiles,
  root,
  opts,
  watch,
  ...
  if (watch) {
    filesWatcher = setupWatchMode({ testFiles, opts });
  } else {
  ...
  return { runFiles, postRun, teardown };
}

// Helper function to run tests in the current process
function runTestsInCurrentProcess({
  testFiles,
  root,
  opts,
  watch,
...
// Helper functions used by runTestFile
function filterExecArgv(arg, i, arr) {
  return !ArrayPrototypeIncludes(kFilterArgs, arg) &&
...
// The FileTest class and other required classes/constants should be included as well
// Ensure they are included in your codebase.
...
function run(options = kEmptyObject) {
  ...
  // Step 3: Initialize test tree
  const root = initializeTestTree({
    concurrency,
    timeout,
    signal,
    setup,
  ...
  // Step 5: Handle isolation modes
  let runFiles;
  let postRun;
  let teardown;
  if (isolation === 'process') {
    ({ runFiles, postRun, teardown } = runTestsInProcessIsolation({
      testFiles,
      root,
      opts,
      watch,
    ...
    ({ runFiles } = runTestsInCurrentProcess({
      testFiles,
      root,
      opts,
      watch,
  ...
  // Step 6: Run setup, tests, and teardown
  const setupPromise = PromiseResolve(setup?.(root.reporter));
  PromisePrototypeThen(
    PromisePrototypeThen(
      PromisePrototypeThen(setupPromise, runFiles),
      postRun
    ),
    teardown
  );

  // Return the test stream (reporter)
