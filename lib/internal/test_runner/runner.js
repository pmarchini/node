...
function runTestFile(path, filesWatcher, opts) {
  ...
  const subtest = opts.root.createSubtest(FileTest, testPath, testOpts, async (t) => {
    ...
    let err;
    // Replace spawn with a worker thread
    const worker = new InternalWorker('internal/test_runner/test_worker', {
      workerData: {
        testPath: testPath,
        cwd: opts.cwd,
      },
      env: {
        ...env,
        NODE_OPTIONS: args.join(' '),
      }
    });

    if (watchMode) {
      filesWatcher.runningProcesses.set(path, worker);
      filesWatcher.watcher.watchChildProcessModules(worker, path);
    }

    worker.on('message', (message) => {
      if (message.type === 'stdout') {
        subtest.parseMessage(Buffer.from(message.data));
      } else if (message.type === 'stderr') {
        subtest.addToReport({
          __proto__: null,
          type: 'test:stderr',
          data: { __proto__: null, file: path, message: message.data + '\n' },
        });
      }
    });
