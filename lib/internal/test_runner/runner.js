...
function runTestFile(path, filesWatcher, opts) {
  ...
  const subtest = opts.root.createSubtest(FileTest, testPath, testOpts, async (t) => {
    ...
    // Check if we should abort before starting the child process
    if (opts.signal?.aborted) {
      const abortError = new Error('Test aborted due to fail-fast');
...
function run(options = kEmptyObject) {
  ...
  if (failFast != null) {
    validateBoolean(failFast, 'options.failFast');
  }
  if (only != null) {
  ...
  if (isolation === 'process') {
    ...
    runFiles = () => {
      ...
      if (failFast) {
        // For fail-fast mode, we need to handle test failures immediately
        let abortController = new AbortController();
        let failed = false;
        
        // Listen for test failures
        const failListener = (event) => {
        ...
        root.reporter.on('data', failListener);
        
        // Create a promise that resolves when all tests complete or fail-fast is triggered
        const executeTests = async () => {
          try {
            const testPromises = ArrayPrototypeMap(testFiles, (path) => {
              const subtest = runTestFile(path, filesWatcher, { ...opts, signal: abortController.signal });
              filesWatcher?.runningSubtests.set(path, subtest);
              return subtest.catch((error) => {
                // If this is an abort error due to fail-fast, don't propagate it
            ...
            await SafePromiseAllSettledReturnVoid(testPromises);
          } finally {
            root.reporter.off('data', failListener);
          }
        };
        
        return executeTests();
      } else {
        // Original behavior for non-fail-fast mode
    ...
    if (watch) {
      ...
      runFiles = async () => {
        ...
        // For fail-fast mode with isolation=none, we need to handle test failures during execution
        if (failFast) {
          let abortController = new AbortController();
          let failed = false;
          ...
          // Combine the abort signal with the existing test signal
          const combinedSignal = opts.signal ? 
            (opts.signal.aborted ? opts.signal : abortController.signal) : 
